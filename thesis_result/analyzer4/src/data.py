...
class SimData:
    ...
    def defineFileName(self):
        ...
        self.file_name = self.config_file_name.split('/')[2].split('.')[0]
    ...
    def regData(self):
        ...
        self.params["numRetransmissions"] = int(self.config["numRetransmissions"])
        ...
        with open(self.config_file_name, 'r') as f:
            for line in f:
                ...
                if key in ["transmitter", "receiver"]:
                    ...
                    fec = FEC(val)
                    self.config[key] = fec
                    if not fec.type == "NONE":
                        self.rate = fec.require_packet / (fec.require_packet * (1 + fec.rate))
                else:
...
class InputData:
    ...
    def parseCollisionFile(self, data):
        ...
        with open(file_name, 'r') as f:
            for line in f:
                _, collision_num = line.split(',')
                self.each_collision.append(collision_num.split('/'))

        self.sum_collisions = 0
        for colls in self.each_collision:
            for coll in colls:
                self.sum_collisions += int(coll)
